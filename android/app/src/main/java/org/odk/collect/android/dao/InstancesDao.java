...
public class InstancesDao {
    ...
    public CursorLoader getSavedInstancesCursorLoader(CharSequence charSequence, String sortOrder) {
        ...
        if (charSequence.length() == 0) {
            ...
            String selection =
                    InstanceColumns.DELETED_DATE + " IS NULL and " + InstanceColumns.STATUS + " !=? AND"
                            + InstanceColumns.DISPLAY_NAME + " LIKE ?";
    ...
    public String getInstancePathForInstanceId(String id) {
        ...
        if(cursor != null && cursor.moveToFirst() &&
            !cursor.isNull(cursor.getColumnIndex(InstanceColumns.INSTANCE_FILE_PATH)))
            return cursor.getString(cursor.getColumnIndex(InstanceProviderAPI.InstanceColumns.INSTANCE_FILE_PATH));

        return null;
    }

    public String getFormIdForInstanceId(String id) {
        String selection = InstanceProviderAPI.InstanceColumns._ID + "=?";
        String[] selectionArgs = {id};
        ...
        if(cursor != null && cursor.moveToFirst() &&
            !cursor.isNull(cursor.getColumnIndex(InstanceProviderAPI.InstanceColumns.JR_FORM_ID)))
            return cursor.getString(cursor.getColumnIndex(InstanceProviderAPI.InstanceColumns.JR_FORM_ID));
