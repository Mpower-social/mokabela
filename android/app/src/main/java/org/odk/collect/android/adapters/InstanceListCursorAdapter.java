...
public class InstanceListCursorAdapter extends SimpleCursorAdapter {
    ...
    private void setUpSubtext(View view) {
        ...
        String submittedBy = getCursor().getString(getCursor().getColumnIndex(InstanceColumns.SUBMITTED_BY));
        String subtext = InstanceProvider.getDisplaySubtext(context, submittedBy, status, new Date(lastStatusChangeDate));
