...
public class FormsDao {
    ...
    public List<String> getFormMediaPaths() {
        List<String> formMediaPaths = new ArrayList<>();

        Cursor cursor = getFormsCursor();

        if (cursor != null && cursor.moveToFirst()) {
            try {
                do {
                    int formMediaPathColumnIndex = cursor.getColumnIndex(FormsProviderAPI.FormsColumns.FORM_MEDIA_PATH);
                    formMediaPaths.add(cursor.getString(formMediaPathColumnIndex));
                } while (cursor.moveToNext());
            } finally {
                cursor.close();
            }
        }
        return formMediaPaths;
    }
    ...
    public void deleteFormByFormId(String formId) {
        String selection = FormsProviderAPI.FormsColumns.JR_FORM_ID + " = ?";
        Collect.getInstance().getContentResolver().delete(FormsProviderAPI.FormsColumns.CONTENT_URI, selection, new String[]{formId});
    }
